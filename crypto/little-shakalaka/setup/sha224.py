...
class SHA224:
    ...
    def digest(self) -> bytes:
        ...
        blocks, _ = self.blockify(
            self._buffer + b"\x80" + b"\x00" * K + M_len.to_bytes(8, "big")
        )
        H_p = self.H.copy()
        for block in blocks:
            self._processing(block)
...
def test():
    ...
    assert (
        ...
        == SHA224(a_str).hexdigest()
    )

    assert (
        "9fc518d2733c281a751c712b244dd01c17f19120ef2ee495e7c7576e"
        == SHA224(a_str * 20).hexdigest()
    )

    s = SHA224()
    s.update(a_str)
    assert "4149da18aa8bfc2b1e382c6c26556d01a92c261b6436dad5e3be3fcc" == s.hexdigest()
    ...
    s.update(a_str)
    s.update(a_str * 8)
    assert "dd542a21a0a399b828b64e6e3e12ac5cfbd1c141e2b2b100b57a93f4" == s.hexdigest()
    ...
    s.update(a_str * 8)
    s.update(a_str * 8)
    assert "5c3acecfbb0ca7893d603bf557faff63aef83b312bf78644caababc5" == s.hexdigest()

    s = SHA224()
    s.update(b"1"*64)
    assert "820518c626796d9af2db299dfb37c8737d8f56f12c44fb99b7aece54" == s.hexdigest()
