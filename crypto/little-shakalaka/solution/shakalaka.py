...
class Shakalaka:
    def __init__(self, M: bytes = b"", H: List[int] = None, L: int= 0):
        if H is None:
            self.H = [
    ...
    @classmethod
    def known_padded_message(cls, known_M: bytes, unknown_len: int) -> bytes:
        M_len = (len(known_M) << 3) + unknown_len
        assert M_len < (2**64)
        K = ((-(M_len + 8 - 0x01C0)) & 0x01FF) // 8
        return known_M + b"\x80" + b"\x00" * K + M_len.to_bytes(8, "little")
