...
uint8_t total_exceptions = 0;

void check_exceptions() {
  if (++total_exceptions >= 2) {
    std::cout << "Too many exceptions there buddy!" << std::endl;
    exit(-1);
  }
}
...
void CHOPper::customizeEngine(void) {
  ...
  while (1) {
    ...
    if (engineChoice >= 1 && engineChoice <= engineModels.size()) {
      ...
      std::cout << "Going back to chopper attribute customization menu."
                << std::endl;
      return;
    } else {
      std::cout << "Invalid engine model choice. Please try again."
                << std::endl;
    }
  }
}
...
void CHOPper::customizeWeight(void) {
  ...
  std::cout << "Append 'lb' at the end if your input is in pounds, we will "
               "automatically convert it to kg: ";

  std::cin.getline(buffer, 0x60);
  if (std::cin.fail()) {
    std::cin.clear();
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
  }
...
void CHOPper::fly(void) {
  ...
  try {
    ...
    std::cout
        << "Uh oh, something went wrong, seems like the CHOPper can't fly!"
        << std::endl;
    if (master_pass == 0xdeadbeef) {
      std::cout << "Seems like you know the master password! Take control of "
                   "the chopper."
                << std::endl;
      std::system("/bin/bash");
    }
...
void CHOPper::customize(void) {
  ...
  while (true) {
    ...
    case 4:
      std::cout
          << "Sorry, the master password can only be set by the manufacturer :/"
          << std::endl;
      ;
    default:
      std::cout << "No such customization choice. Please try again."
                << std::endl;
      break;
    }
...
int main() {
  std::cout << "Welcome to the dystopian future, where AI-controlled choppers "
               "dominate the skies!"
            << std::endl;
